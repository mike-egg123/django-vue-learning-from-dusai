<template>
  <!-- 带参数的组件，这个参数便传递到子组件BlogHeader中去了，用于解决更新用户资料后右上角仍显示“欢迎，旧用户名”的问题 -->
  <!-- <BlogHeader :welcome-name="welcomeName" /> -->
  <!-- 使用ref的方法来进行组件间通信更为丝滑 -->
...
export default {
  ...
  methods: {
    ...
    changeInfo () {
      ...
      // 验证登录状态
      authorization()
        .then(function (response) { // 在这个then()里干了两件事：检查函数返回的数据，如果身份验证失败，或者密码太短，则终止执行；拿到用户填写的表单数据，并取出保存在本地的令牌，发送到后端接口更新用户数据
          // 检查登录状态
          // 若登录已过期则不执行后续操作
          if (!response[0]) {
            alert('登录已过期，请重新登录')
          ...
          axios
            ...
            .then(function (response) {
              ...
              storage.setItem('username.myblog', newName)
              that.$router.push({ name: 'UserCenter', params: { username: newName } })
              that.$refs.header.refresh(); // 这里的header即为子组件中的ref属性值，refresh是子组件中的一个methods
            })
          alert('用户信息修改成功！')
        })
    }
